...
let rec calc_size (delta : int) fs =
    match fs with
    | Dir d ->
        let my_size =
            String.Map.fold ~init:0 ~f:(fun ~key:_ ~data total ->
                match data with
                | Dir _ as c -> total + calc_size delta c
                | File f -> total + f.size
            ) d.files
        in
        if my_size >= delta && String.(d.name <> "/") then (
            Printf.printf "--> %10s %d\n" d.name my_size;
        );
        my_size
    | File f -> f.size
...
let _ =
    ...
    |> (fun r -> 
        print r;
        let total = total_size r in
        let delta = max_size - total in
        Printf.printf "Extra %d\n%!" delta;
        Printf.printf "%d \n" (calc_size (min_free - delta) r);
    )
